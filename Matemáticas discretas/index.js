...
function unirNodos(graph, n, m) {
  ...
  function generarGrafo() {
    ...
    const visNodes = new vis.DataSet(Object.keys(grafoResultante).map(node => ({ id: node, label: `${node}`})));
    ...
    Object.keys(grafoResultante).forEach(node => {
      ...
      neighbors.forEach(neighbor => {
        visEdges.add([
          { from: node, to: neighbor, arrows: { to: { enabled: true, scaleFactor: 0.8 } } },
          { from: neighbor, to: node,arrows: { to: { enabled: true, scaleFactor: 0.8 } } }
        ]);
      });
    });
